...
Public Class PrintForm
    ...
    Private Sub PrintForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            command.CommandText = "SELECT pcc.pcc_id, [pcc].[pcc_number], [applicant].[fname], [applicant].[mname], [applicant].[lname], [applicant].[place_of_birth], [applicant].[date_of_birth], [applicant].[sex], [applicant].[height], [applicant].[nationality], [applicant].[contact_no], [pcc].[ctc_number], [pcc].[ctc_issued_on], [pcc].[ctc_issued_at], [pcc].[signature], [pcc].[img], [pcc].[thumb], [pcc].[pcc_issue_date], [pcc].[payment_amount], [pcc].[payment_or_number], [zone].[name] AS 'zone_name', [barangay].[name] AS 'barangay_name', [civil_status].[name] AS 'cs_name', [purpose].[name] AS 'purpose_name', police_verify.fname AS 'police_vfname', police_verify.mname AS 'police_vmname', police_verify.lname AS 'police_vlname', police_verify_position.name AS 'police_vposition', police_verify.police_sig AS 'police_vsig', police_certify.fname AS 'police_cfname', police_certify.mname AS 'police_cmname', police_certify.lname AS 'police_clname', police_certify_position.name AS 'police_cposition', police_certify.police_sig AS 'police_csig', pcc.cr_id, pcc.findingsRemarks, criminal_records.crime_offense, police_verify_rank_table.name AS 'police_vrank', police_certify_rank_table.name AS 'police_crank' FROM pcc INNER JOIN [dbo].[applicant] ON [pcc].[applicant_id] = [applicant].[applicant_id] INNER JOIN [zone] ON [applicant].zone_id = [zone].zone_id INNER JOIN [barangay] ON [applicant].barangay_id = [barangay].barangay_id INNER JOIN [civil_status] ON [applicant].cs_id = [civil_status].cs_id INNER JOIN [purpose] ON [pcc].purpose_id = [purpose].purpose_id INNER JOIN [police] police_verify ON [pcc].police_id_verify = police_verify.police_id INNER JOIN [police] police_certify ON [pcc].police_id_certify = police_certify.police_id LEFT JOIN (SELECT police_id, curr_rank_id, ROW_NUMBER() OVER (PARTITION BY police_id ORDER BY promotion_date DESC) AS rn FROM promotion) police_verify_rank ON police_verify.police_id = police_verify_rank.police_id AND police_verify_rank.rn = 1 LEFT JOIN (SELECT police_id, curr_rank_id, ROW_NUMBER() OVER (PARTITION BY police_id ORDER BY promotion_date DESC) AS rn FROM promotion) police_certify_rank ON police_certify.police_id = police_certify_rank.police_id AND police_certify_rank.rn = 1 LEFT JOIN dbo.[rank] AS police_verify_rank_table ON police_verify_rank.curr_rank_id = police_verify_rank_table.rank_id LEFT JOIN dbo.[rank] AS police_certify_rank_table ON police_certify_rank.curr_rank_id = police_certify_rank_table.rank_id INNER JOIN [position] police_verify_position ON police_verify.position_id = police_verify_position.position_id INNER JOIN [position] police_certify_position ON police_certify.position_id = police_certify_position.position_id LEFT JOIN criminal_records ON criminal_records.cr_id = pcc.cr_id WHERE [pcc].[pcc_id] = " & pcc_id
